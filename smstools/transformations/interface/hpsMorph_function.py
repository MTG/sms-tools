...
def analysis(
    ...
    plt.pcolormesh(
        frmTime,
        binFreq,
        np.transpose(stocEnv1[:, : int(sizeEnv * maxplotfreq / (0.5 * fs1)) + 1]),
    )
...
def transformation_synthesis(
    ...
    hmag2,
    stocEnv2,
    hfreqIntp=np.array([0, 0, 0.1, 0, 0.9, 1, 1, 1]),
    hmagIntp=np.array([0, 0, 0.1, 0, 0.9, 1, 1, 1]),
    stocIntp=np.array([0, 0, 0.1, 0, 0.9, 1, 1, 1]),
):
    """
    ...
    yhfreq, yhmag, ystocEnv = HPST.hpsMorph(
...
if __name__ == "__main__":
    # analysis
    inputFile1, fs1, hfreq1, hmag1, stocEnv1, inputFile2, hfreq2, hmag2, stocEnv2 = (
        analysis()
    )
