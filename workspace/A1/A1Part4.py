...
def downsampleAudio(inputFile, M):
    ...
    """
    ## Your code here
    input_name_extension = os.path.basename(inputFile)
    input_name = os.path.splitext(input_name_extension)[0]

    ori_rate, ori_audio_file_lst = wavread(inputFile)
    new_audio_file_lst = hopSamples(ori_audio_file_lst, M)
    new_rate = ori_rate // M

    outputAudio_name = input_name + '_downsampled.wav'
    outputAudio_path = '../../sounds/' + outputAudio_name
    wavwrite(new_audio_file_lst, new_rate, outputAudio_path)
