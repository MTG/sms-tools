...
plt.subplot(311)
plt.plot(np.arange(pin, pin+w.size)/float(fs), x1, 'b-x', ms=3, lw=1.5)
plt.axis([pin/float(fs), (pin+w.size)/float(fs), min(x1), max(x1)])
plt.title('x (sine-440-490.wav)')

plt.subplot(3,1,2)
plt.plot(fs*np.arange(mX.size)/float(N), mX, 'r-x', ms=3, lw=1.5)
plt.axis([0,1200,-80,max(mX)+1])
plt.title ('mX')

plt.subplot(3,1,3)
plt.plot(fs*np.arange(mX.size)/float(N), pX, 'c-x', ms=3, lw=1.5)
