...
class SprModel_frame:
    ...
    def initUI(self):
        ...
        self.compute.grid(row=18, column=0, padx=5, pady=(20,2), sticky=W)

        #BUTTON TO PLAY SINE OUTPUT
        output_label = "Sinusoidal:"
        Label(self.parent, text=output_label).grid(row=19, column=0, sticky=W, padx=5, pady=(20,15))
        self.output = Button(self.parent, text=">", command=lambda:self.play_out_sound('sprModel_sines'), bg="gray30", fg="white")
        self.output.grid(row=19, column=0, padx=(75,5), pady=(20,15), sticky=W)

        #BUTTON TO PLAY RESIDUAL OUTPUT
        output_label = "Residual:"
        Label(self.parent, text=output_label).grid(row=20, column=0, sticky=W, padx=5, pady=(5,15))
        self.output = Button(self.parent, text=">", command=lambda:self.play_out_sound('sprModel_residual'), bg="gray30", fg="white")
        self.output.grid(row=20, column=0, padx=(75,5), pady=(5,15), sticky=W)

        #BUTTON TO PLAY OUTPUT
        output_label = "Output:"
        Label(self.parent, text=output_label).grid(row=21, column=0, sticky=W, padx=5, pady=(5,15))
        self.output = Button(self.parent, text=">", command=lambda:self.play_out_sound('sprModel'), bg="gray30", fg="white")
        self.output.grid(row=21, column=0, padx=(75,5), pady=(5,15), sticky=W)
